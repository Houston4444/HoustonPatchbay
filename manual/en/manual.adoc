...
+
This is a setting for the automatic layout (default). Some people much prefer wide boxes, but there are cases where a high box saves a tremendous amount of space, for example, when there is only one input port and ten output ports. The multi-choice box is actually a dosing:

** *Choose the smallest area*: The boxes will be just as likely to be high as they are to be large.  
** *Prefer large boxes*: The high layout must offer a significant advantage to be chosen
** *Almost only large boxes*: All the boxes will have a large layout, unless a high layout offers a clearly obvious space-saving advantage
** *Force large boxes*: For those who can't stand the high layout
