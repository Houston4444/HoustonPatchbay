...
:toc: left
:toclevels: 2
:imagesdir: ../../../images/patchbay/common
...
Note that aliases _audio_, _midi_, _cv_, _alsa_ and _video_ are now required for the view selector frame. Please affect them a color.
...
* *port-height* : <<number>>
* *port-grouped-width*: <<number>>, minimum width of grouped ports
...
* *border-color* : <<color>>
* *border-mode*: _default_ or _minimal_ can draw only few borders, as in *Terminal* theme
...
* *font-weight* : <<number>> between 1 (very light) and 1000 (very bold). Or, with a large preference, it can also be:
...
* *text-color* : <<color>>
* *visible* : <<boolean>>. Few blocks support this.
* *drilled* : <<boolean>>. Few blocks support this. If _True_, the parent widget is drilled. Useful only if background-color has transparency.
* *grid-min-width* : <<number>>, in *grid* block only
* *grid-min-height* : <<number>>, in *grid* block only
* *inherits* : only for *portgroup* block, can be _port_ to inherit all *port* settings.
...
*header* and *gui_button* can also interpret some margin values:
If the box has title on top:

* *margin-bottom* : <<number>> margin on bottom
* *margin-top* : <<number>> margin on top
* *margin-sides*: <<number>> margin on left and on right

If the box has title on side:

* *margin-top-side* : <<number>> margin on top and bottom
* *margin-ports-side*: <<number>> margin on the side which contains the ports
* *margin-free-side* : <<number>> margin on the opposite side of the ports

You can also apply in all cases

* *margin* : <<number>>. Set all *margin-...* attributes to value
...
box_wrapper.client.with_gui
box_wrapper.client.with_gui.selected

box_header
box_header.selected
box_header.hardware
box_header.hardware.selected
box_header.monitor
box_header.monitor.selected
box_header.client
box_header.client.selected
box_header.client.with_gui
box_header.client.with_gui.selected
...
box_header_line.client.with_gui
box_header_line.client.with_gui.selected
...
box_shadow.client.with_gui
box_shadow.client.with_gui.selected

box_ports_border
box_ports_border.selected
box_ports_border.hardware
box_ports_border.hardware.selected
box_ports_border.monitor
box_ports_border.monitor.selected
box_ports_border.client
box_ports_border.client.selected
box_ports_border.client.with_gui
box_ports_border.client.with_gui.selected
...
Note that if the audio port is selected, it will check if "selected" block of the parent block exists and search inside before. The order of search will be:
...
* *output-align*: _left_ or _right_, determines if the output ports names should be aligned on left or on right
* *icon-size* : <<number>>, default is 24
* *port-in-offset*: <<number>>, the horizontal offset of the input ports from the border of the box
* *port-out-offset*: <<number>>, the horizontal offset of the output ports from the border of the box
* *port-offset*: <<number>>, set *port-in-offset* and *port-out-offset*
* *port-in-offset-mode* : <<number>>, _bore_ or _none_, determinates if the box should be bored with port-offset (for input ports)
* *port-out-offset-mode* : <<number>>, _bore_ or _none_, determinates if the box should be bored with port-offset (for output ports)
* *port-offset-mode* : <<number>>, set *port-in-offset-mode* and *port-out-offset-mode*
* *port-spacing* : <<number>>, vertical spacing between ports (if they are not in the same portgroup)
* *port-type-spacing*: <<number>>, vertical spacing between ports of different type, in addition to *port_spacing*
...
[#boolean]
=== boolean
_boolean_ is _True_ or _False_.
